...
def run():
    ...
    select_type = st.pills("Use RTT code or start and end stations?", options = ["RTT Number", "Stations"], default = "RTT Number")
    Data = all_data()
    ...
    with st.expander("View all waypoints to be plotted:"):
    ...
    if st.session_state.allcalls is None:
        ...
        with st.expander("Choose plot parameters:"):
            ...
            trange_min, trange_max = st.slider("Time range (hours):", min_value = 0., max_value = 24., value = (max(0., current_time/60. - 1.), min(24., current_time/60. + 1.)))
            tmin = trange_min*60; tmax = trange_max*60; current_time = st.session_state.timeref
            dot_time = st.slider("Time to plot RT until", min_value = trange_min, max_value = trange_max + 1, value = trange_max + 0.1)
            plot_up = st.checkbox("Plot trains going up the plot", value = True)
            plot_down = st.checkbox("Plot trains going down the plot", value = True)
            plot_rt = st.checkbox("Plot actual times", value = True)
            plot_wtt = st.checkbox("Plot scheduled times", value = True)

        plot_trains(xmin = tmin, xmax = tmax, counter = -1, dot_time = dot_time*60, operators = True, plot_up = plot_up, plot_down = plot_down, plot_rt = plot_rt, plot_wtt = plot_wtt)
